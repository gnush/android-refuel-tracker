...
import io.github.gnush.refueltracker.data.VolumeDao
...
class VolumeDaoTest {
    private lateinit var dao: VolumeDao
    ...
    private val row1 = Volume(1, "L")
    private val row2 = Volume(2, "kWh")
    private val rows = listOf(row1, row2)

    private suspend fun addRowsToDb(rows: List<Volume>) = rows.forEach {
        dao.insert(it)
    }
    ...
    fun daoInsert_insertsIntoDb() = runBlocking {
        ...
        val res = db.query("select * from volume", null)
    ...
    fun daoUpdate_updatesInDb() = runBlocking {
        ...
        val symbol = "SYMBOL"
        dao.update(Volume(row1.id, symbol))
